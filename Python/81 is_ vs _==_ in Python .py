# In Python, is and == are both comparison operators that can be used to check if two values are equal. However,
# there are some important differences between the two that you should be aware of.
...
# In this case, a and b are two separate lists that have the same values, so == returns True. However, a and b
# are not the same object in memory, so is returns False.
...
# For mutable objects such as lists and dictionaries, is and == can behave differently. In general, 
# you should use == when you want to compare the values of two objects, and use is when you want to check if 
